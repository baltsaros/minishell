...
/*   Updated: 2022/07/14 16:40:36 by mthiry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_size_cmd(t_node	*args)
{
	...
	// 	if (args->type == ASTER)
	// 		i++;
	// 	else if (args->type == SLASH)
	// 	{
	// 		if (args->prev && args->next && args->prev->type == WSPACE && args->next->type == WSPACE)
	// 			i++;
	// 		else if (!args->prev && args->next && args->next->type == WSPACE)
	// 			i++;
	// 		else if (args->prev && !args->next && args->prev->type == WSPACE)
	// 			i++;
	// 		else if (!args->prev && !args->next)
	// 			i++;
	// 	}
...
char	**init_cmd(t_node *args, t_input *data)
{
	...
	while (args && i != size && args->type != PIPE)
	{
		if (args->type != QUOTE_D && args->type != QUOTE)
		{
			...
			// 		if (!str[i])
			// 			return (NULL);
			// 		if (args->next && args->next->type == SLASH)
			// 		{
			// 			args = args->next;
			// 			while (args && args->type != WSPACE)
			// 			{
			// 				str[i] = ms_strjoin_free(str[i], args->value, data);
			// 				if (!str[i])
			// 					return (NULL);
			...
			// 	{
			// 		args = args->next;
			// 		str[i] = ms_strdup(get_env_variable(args->value), data);
			// 		if (args->next && args->next->type == SLASH)
			// 		{
			// 			args = args->next;
			// 			while (args && args->type != WSPACE)
